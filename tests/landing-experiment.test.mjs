...
const source=fs.readFileSync(new URL('../vsl/experiment.js',import.meta.url),'utf8');
...
for(const [random,variant] of [[0,'A'],[0.49999,'A'],[0.5,'B'],[0.9999,'B']]){
 const first=run('es/?'+campaign,{random});assert.equal(new URL(first.redirected).pathname,new URL('es/'+variant.toLowerCase()+'/',base).pathname);
 ...
 const checkout=landing.exp.checkout(new URL('https://pay.hotmart.com/T107441078P?'+campaign));assert.equal(checkout.searchParams.get('sck'),'120250252288070633_ab0906'+variant);
...
for(const path of ['es/','es/?meta_ad_id=other&meta_campaign_id=120250216188090633','es/?preview=1&'+campaign,'es/b/?preview=1','es/b/']){const r=run(path);assert.equal(r.redirected,undefined);assert(!r.exp.enrolled)}
const preview=run('es/b/?preview=1');assert(preview.exp.preview);assert.equal(preview.exp.checkout(new URL('https://pay.hotmart.com/T107441078P')).searchParams.get('sck'),'preview_ab0906B');
const invalid=new Map([['pr_ab_20260906_session','{"variant":"evil","started":0}']]);assert(run('es/?'+campaign,{store:invalid}).redirected.endsWith('a/?'+campaign));
