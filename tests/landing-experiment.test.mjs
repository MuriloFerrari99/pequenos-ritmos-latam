...
const source=fs.readFileSync(new URL('../vsl/experiment-r2.js',import.meta.url),'utf8');
...
for(const [random,variant] of [[0,'A'],[0.49999,'A'],[0.5,'B'],[0.9999,'B']]){
 const first=run('es/?'+campaign,{random});assert.equal(new URL(first.redirected).pathname,new URL('es/'+variant.toLowerCase()+'2/',base).pathname);
 ...
 const checkout=landing.exp.checkout(new URL('https://pay.hotmart.com/T107441078P?'+campaign));assert.equal(checkout.searchParams.get('sck'),'120250252288070633_ab0906r2'+variant);
...
for(const path of ['es/','es/?meta_ad_id=other&meta_campaign_id=120250216188090633','es/?preview=1&'+campaign,'es/b2/?preview=1','es/b2/']){const r=run(path);assert.equal(r.redirected,undefined);assert(!r.exp.enrolled)}
const preview=run('es/b2/?preview=1');assert(preview.exp.preview);assert.equal(preview.exp.checkout(new URL('https://pay.hotmart.com/T107441078P')).searchParams.get('sck'),'preview_ab0906r2B');
const invalid=new Map([['pr_ab_20260906_r2_session','{"variant":"evil","started":0}']]);assert(run('es/?'+campaign,{store:invalid}).redirected.endsWith('a2/?'+campaign));
...
assert(!run('es/b2/',{store:previousRound}).exp.enrolled,'old experiment assignment cannot classify a new-round page');
const newRound=run('es/?'+campaign,{store:previousRound,random:0.2});
assert(newRound.redirected.includes('/a2/'),'new round gets its own assignment');
assert(previousRound.has('pr_ab_20260906_session'),'old attribution remains available for already open pages');
assert(previousRound.has('pr_ab_20260906_r2_session'));
