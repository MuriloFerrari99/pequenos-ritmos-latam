...
const routes = ['pt/', 'es/', 'es/a/', 'es/b/', 'es/a2/', 'es/b2/', 'es/a3/', 'es/b3/', 'es/mx/', 'es/co/', 'es/cl/', 'es/pe/', 'es/ar/'];
...
for (const base of ['https://muriloferrari99.github.io/pequenos-ritmos-latam/vsl/', 'http://localhost:8080/vsl/']) {
  for (const route of routes) {
    ...
    for (const suffix of ['', 'index.html']) {
      ...
      assert.equal(policies.length, ['es/','es/a2/','es/b/','es/b2/','es/a3/','es/b3/'].includes(route) ? 4 : 3);
...
console.log('PASS: eleven country/language/experiment routes, both policies, consent policy link, logo query, production/local base, index.html paths, campaign retention, prohibited fields and invalid redirect rejection.');
